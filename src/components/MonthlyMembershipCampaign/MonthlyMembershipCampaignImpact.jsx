  ...
  <section className="membership-section membership-impact" aria-labelledby="impact-title">
    <div className="membership-shell">
      ...
      <div className="membership-impact__grid">
        {impact.map((item) => <article className="membership-impact__card" key={item.title}><div className="membership-impact__photo"><img src={sadaqaImage} alt="MTJF sadaqa impact" /></div><div><h3>{item.title}</h3><p>{item.detail}</p></div></article>)}
      </div>
    </div>
  </section>
)
